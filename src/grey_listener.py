...
#
# This script reads raw pictures from grey_scale_topic
...
def callback(data): # runs everytime an ImageMessage is uploaded on the topic
    ...
    cv2.imshow("Rospibot Greyscale Image", img) # display the image
    cv2.waitKey(2) # keep displaying the image

def listener():

    rospy.init_node('grey_scale_listener', anonymous=True) # create a image_converter_listener node

    rospy.Subscriber("grey_scale_topic",Image,callback,queue_size=1) # subscribe to image_topic
